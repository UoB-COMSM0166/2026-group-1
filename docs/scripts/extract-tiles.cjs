 ...
 *
 * Run: node scripts/extract-tiles.js
 */

const fs   = import fs from "fs";
const path = import path from "path";
const sharp = import sharp from "sharp";
const { XMLParser } = import { XMLParser } from "fast-xml-parser";
...
// ── step 1: collect all GIDs used in all rooms ────────────────────────────────
...
const tilesetBySource = {}; // source path → { firstgid, name }

for (const rf of roomFiles) {
  const room = readJson(path.join(ROOMS_DIR, rf));
...
// ── step 2: resolve each GID → its tileset + local ID ───────────────────────
...
// atlasUsage: source → Set of local IDs needed
...
// ── step 3: parse TSX files ─────────────────────────────────────────────────

const atlasMeta = {}; // source → { imagePath, tilewidth, tileheight, columns, firstgid, name }

for (const [src, localIds] of atlasUsage) {
  // src is like "../tilesets/prototype-tileset/prototype-tileset.tsx"
  // resolve relative to data/rooms/ → data/tilesets/...
  const tsxAbs = path.resolve(DOCS, 'data', 'rooms', '..', src);
  ...
  const meta = parseTsx(fs.readFileSync(tsxAbs, 'utf8'), src);
  if (!meta?.imagePath) {
...
(async () => {
  ...
  for (const [src, localIds] of atlasUsage) {
    ...
    const atlasAbs = path.join(DOCS, meta.imagePath);
    ...
    try {
      const raw = await sharp(atlasAbs)
        .ensureAlpha()
        .raw()
    ...
    console.log(`  ${meta.name}: ${atlasW}x${atlasH}, extracting ${sortedLocals.length} tiles...`);
    ...
    for (const localId of sortedLocals) {
      ...
      if (sx + meta.tilewidth > atlasW || sy + meta.tileheight > atlasH) {
        console.warn(`    SKIP GID ${globalGid}: out of bounds (${atlasW}x${atlasH})`);
        skipped++;
        continue;
      }
  ...
  console.log('\nRenderSystem change needed:');
  console.log('  1. Load tiles as: assets[`tile:${gid}`] = loadImage(`data/tiles/${tileset.name}/${gid}.png`)');
  console.log('  2. Draw with:      image(assets[`tile:${gid}`], x, y, w, h);');
  console.log('  (no more atlas slicing, direct GID → filename lookup)');
